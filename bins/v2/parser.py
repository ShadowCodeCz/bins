...
class TemplateParser:
    def __init__(self, sub_templates, name=""):
    ...
    def parse(self, binary_value):
        self.predefined_blocks = generate_blocks_from_templates(self.sub_templates, binary_value)
...
class Parser:
    def __init__(self):
        self.templates = template.all
        self.parsers = {name: TemplateParser(sub_templates, name) for name, sub_templates in self.templates.items()}

    def parse(self, parser_name, binary_value):
        return self.parsers[parser_name].parse(binary_value)
